...
function gameOver(num){
    ...
    if(num == 1){
        setTimeout(opponent_death_animation.bind(null,enemy_poke["poke_id"]), 1500)
...
function startBattle() {
    ...
    document.getElementById("tomb").style.visibility = "hidden"
...
function push_poke() {
    ...
    for (var ctr = 0; ctr < my_poke.length; ctr++) {
        ...
        // player_id.push(player_id[0])
        poke_id.push(my_poke[ctr]["poke_id"])
        
        hp.push(my_poke[ctr]["hp"])
        xp.push(parseInt(my_poke[ctr]["xp"]) + 1)
        console.log(parseInt(my_poke[ctr]["xp"]) + 1)
    }

    console.log(player_id)
    currMoney = parseInt(currMoney) + (1 * enemy_poke["xp"])

    $.ajax({
        type: "POST",  //type of method
        url: "php/send_player_poke.php",  //your page
        data: {player_id: player_id, poke_id: poke_id, xp: xp, hp: hp, name: p_currName, money: currMoney },// passing the values
